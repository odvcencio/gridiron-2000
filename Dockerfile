...
# v0.50.0 includes the native document contract, nonce-aware navigation,
# static bearer middleware, File/Files, and MaxActionBodyBytes for managed actions. The
# avatar route still keeps its outer multipart envelope cap until the
# production consumer adopts a bounded-multipart contract.
RUN go install m31labs.dev/gosx/cmd/gosx@v0.50.0 && GOSX_SKIP_VERSION_CHECK=1 /go/bin/gosx build --dev .
