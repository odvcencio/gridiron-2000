...
# v0.55.1 includes a Scene3D fill-height layout fix (unused by this app),
# soft navigation that reconciles the body in place instead of replacing it
# wholesale, selective runtime asset emission, and bundle-budget review
# bands. It does NOT include comment stripping inside .gsx markup at
# compile time; that fix shipped only on a v0.53.11 maintenance pseudo-
# version this app pinned earlier and has not yet reached the v0.55 line.
...
RUN go install m31labs.dev/gosx/cmd/gosx@v0.55.1 && GOSX_SKIP_VERSION_CHECK=1 /go/bin/gosx build --dev .
