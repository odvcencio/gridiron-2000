...
func (s *Service) DashboardData(ctx context.Context, r *http.Request) map[string]any {
	...
	livePoll := livePollingActive(live)
	return map[string]any{
		"viewer":                viewer,
		...
		"live":                  s.liveMap(live),
		"live_interval":         map[bool]string{true: "10s", false: ""}[livePoll],
		"live_poll":             livePoll,
		"featured":              featured,
...
func (s *Service) teamData(r *http.Request, readOnly bool) map[string]any {
	...
	// startersHasProjection (section-B item 2, section-C item 8): projected
	// (just above, TeamStartersProjectedTotal(lineup)) is the ONE
	// starters-only figure the stat strip, the current-matchup card just
	...
	data := map[string]any{
		...
		// projected (section-B item 2 / section-C item 8 truth fix) is now
		// starters only — TeamStartersProjectedTotal(lineup), the one
		// canonical helper /matchups' own featured card and this page's
		// current-matchup card both agree with — never a sum over the
		// whole roster (bench, reserve, and IR included), which is what
		// made this figure disagree with /matchups for the same team and
		// week.
		"projected":          projectedText(projected, startersHasProjection),
...
func (s *Service) liveScoresView(live LiveSnapshot, isCurrentWeek bool, currentWeek int) map[string]any {
	...
	// for the whole poll, so every row's PROJ recomputes from the same
	// starterProjectedText math the initial render used, and a poll can
	// never leave PROJ stale while PTS keeps ticking beside it.
	starterProjBind := make(map[string]string)
	// starterOriginalProjBind is the source weekly forecast shown by the
	// Matchups UI. It intentionally stays separate from starterProjBind's
	// rest-of-game estimate so the reference number does not change after a
	// game reaches FINAL.
	starterOriginalProjBind := make(map[string]string)
	...
	// stillToPlaySentenceBind is A1's plain-words rendering of the same
	// count (matchup redesign 2026-09-07), kept live-bound alongside the
	...
	addStarterRow := func(row StarterLedgerRow) {
		...
		starterOriginalProjBind[row.LiveKey] = originalStarterProjectedText(row, projectionByID)
	}
...
// livePollingActive is the browser-facing poll gate. A scheduled week can
// render its kickoff time without repeatedly asking the live endpoint, and a
// gap between NFL windows is just as quiet: SlateLine is empty unless a game
// is inside the active scoreboard window. Degraded/final/preseason snapshots
// remain available on demand but never create a background request loop.
func livePollingActive(live LiveSnapshot) bool {
	return live.State == MatchupStateInProgress && strings.TrimSpace(live.SlateLine) != ""
}
...
func matchupPresentation(state string, pollerEnabled bool) map[string]string {
	...
	case MatchupStateScheduled:
		return map[string]string{
			"headline_top": "WEEK", "headline_bottom": "SCHEDULED.",
			"sync_label": "Waiting for kickoff", "refresh_label": "Quiet until kickoff",
			"note_title": "Scheduled scoring", "note_body": "Live requests stay off until a game is underway. Open or refresh this page once kickoff begins.",
		}
	case MatchupStateInProgress:
		if !pollerEnabled {
			return map[string]string{
		...
		return map[string]string{
			"headline_top": "LIVE", "headline_bottom": "SIGNAL.",
			"sync_label": "Live scores on", "refresh_label": "Live updates",
	...
	case MatchupStateDegraded:
		return map[string]string{
			"headline_top": "SCHEDULE", "headline_bottom": "STATUS.",
			"sync_label": "Timing unavailable", "refresh_label": "Manual refresh",
...
func (s *Service) matchupMaps(state PersistedState, matchups []ScoreMatchup) []map[string]any {
	...
	for _, matchup := range matchups {
		...
		_, awayHasLargeImage, awayAvatarLargeURL := s.avatarViewLarge(away.ID, away.Tone)
		if !awayHasLargeImage {
			awayAvatarLargeURL = awayAvatarURL
		}
		_, homeHasLargeImage, homeAvatarLargeURL := s.avatarViewLarge(home.ID, home.Tone)
		if !homeHasLargeImage {
			homeAvatarLargeURL = homeAvatarURL
		}
		...
		out = append(out, map[string]any{
			...
			"away": map[string]any{
				...
				"has_avatar": awayHasAvatar, "has_avatar_image": awayHasImage, "avatar_image_url": awayAvatarURL, "avatar_image_large_url": awayAvatarLargeURL, "team_href": "/team?team=" + url.QueryEscape(away.ID),
			},
			"home": map[string]any{
				...
				"has_avatar": homeHasAvatar, "has_avatar_image": homeHasImage, "avatar_image_url": homeAvatarURL, "avatar_image_large_url": homeAvatarLargeURL, "team_href": "/team?team=" + url.QueryEscape(home.ID),
			},
			"status": matchup.Status,
...
func (s *Service) featuredMatchupViews(state PersistedState, live LiveSnapshot, matchups []map[string]any, teamID, focusID string, viewedWeek, lockWeek int, projectionByID map[string]Player, projectionAvailable bool) (map[string]any, []map[string]any) {
	...
	for i, entry := range matchups {
		...
		if i < len(live.Matchups) {
			...
			stillToPlayCount := stillToPlay(combined, status)
			entry["still_to_play"] = stillToPlayCount
			entry["still_to_play_total"] = len(combined)
			// still_to_play_sentence is A1's plain-words rendering of the
			// same count (matchup redesign 2026-09-07), retiring the "N of M
...
func (s *Service) featuredMatchupMap(state PersistedState, m ScoreMatchup, isViewer bool, teamID string, viewedWeek, lockWeek int, status LiveStatus, hasLive bool, byID map[string]Player, projectionAvailable bool) map[string]any {
	...
	stillToPlayCount := stillToPlay(combined, status)
	// mine_bench/theirs_bench (A4, matchup redesign 2026-09-07): the
	// Benches disclosure lists both benches with PROJ — public information
	// in this league, since it explains why a manager is favoured.
	// effectiveLineupForTeam reads the CURRENT roster's bench for
...
// benchRowMaps converts an EffectiveLineup's Bench into the Benches
// disclosure's own row shape (A4, matchup redesign 2026-09-07): bench
// composition is public information in this league (it explains why a
// manager is favoured), so every row is a plain name/position/team/proj —
...
// starterLedgerMapsWithProjection decorates starterLedgerMaps' own output
// with each row's PROJ cell (A2, matchup redesign 2026-09-07): the slot
// table's per-starter PROJ column, which must sum to exactly the same
// projected total the header card already shows for that side, so this
// reuses projectedTotal's identical per-row math (starterProjectedText)
// rather than a second, potentially-diverging formula. byID is the
// caller's single s.pool().byID read for the whole render (see
// featuredMatchupViews).
func starterLedgerMapsWithProjection(rows []StarterLedgerRow, byID map[string]Player, status LiveStatus, hasLive bool) []map[string]any {
	maps := starterLedgerMaps(rows)
	for i, row := range rows {
		maps[i]["proj"] = starterProjectedText(row, byID, status, hasLive)
		maps[i]["original_proj"] = originalStarterProjectedText(row, byID)
	}
	return maps
}
...
func featuredStarterPairs(mineRows, theirsRows []StarterLedgerRow, byID map[string]Player, status LiveStatus, hasLive bool) []map[string]any {
	mine := starterLedgerMapsWithProjection(mineRows, byID, status, hasLive)
	theirs := starterLedgerMapsWithProjection(theirsRows, byID, status, hasLive)
