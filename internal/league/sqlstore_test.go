...
func TestV6SQLiteMigratesWaiverReceiptsToV7(t *testing.T) {
	...
	// and the last kv schema_version stamp
	// (migrate013CommissionerEventReleasedEmails, F8) — not
	// currentDBVersion's own step count — is the logical marker to check;
	// migrate010/migrate011 stamp no logical version at all (see their
	// own doc comments).
	var userVersion int
	...
	if err := db.QueryRow(`SELECT value FROM kv WHERE key = ?`, kvSchemaVersion).Scan(&logicalVersion); err != nil || logicalVersion != "12" {
		t.Fatalf("logical schema = %q (err %v), want 12", logicalVersion, err)
	}
	var receiptTable string
