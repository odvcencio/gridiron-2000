...
func TestDraftDataReadOnlyDoesNotProvisionAuthenticatedViewer(t *testing.T) {
	...
	handler := authn.Middleware(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		data = service.DraftDataReadOnly(request)
		writer.WriteHeader(http.StatusNoContent)
	}))
	handler.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/draft/fragment/room", nil))
	after := service.store.Snapshot()
	if !reflect.DeepEqual(before, after) {
		t.Fatalf("read-only draft data mutated persistence\nbefore: %#v\n after: %#v", before, after)
	}
	viewer, _ := data["viewer"].(map[string]any)
	if viewer["signed_in"] != true || viewer["has_seat"] != false || viewer["email"] != "observer@example.com" {
		t.Fatalf("read-only viewer = %#v", viewer)
	}
}
