...
// Checked in both render copies (Page() and ActivityRegion) since only
// one is ever in the DOM for a given request (the fragment swap target).
func TestActivityFilterRailIsStickyWithTopPaginationAndBackToTop(t *testing.T) {
	...
	if got := strings.Count(source, `<div class="pool-filter-rail" id="activity-filters">`); got != 2 {
		t.Errorf(`page.gsx has %d copies of the activity filter rail wrapper, want 2 (Page() and ActivityRegion())`, got)
	}
	if got := strings.Count(source, `<a class="access-link activity-back-to-top" href="#activity-filters">`); got != 2 {
		t.Errorf(`page.gsx has %d back-to-top links, want 2 (Page() and ActivityRegion())`, got)
	}
	if got := strings.Count(source, `class="pool-pagination pool-pagination--top"`); got != 2 {
		t.Errorf(`page.gsx has %d top-of-list pagination navs, want 2 (Page() and ActivityRegion())`, got)
	}
	for _, want := range []string{
		...
		if !strings.Contains(source, want) {
			t.Errorf("page.gsx missing search-field contract %q", want)
		}
	}
