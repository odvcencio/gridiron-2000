	...
	return <main class="page guide-page" id="main-content">
		...
		<section class="guide-section guide-section--accent" id="quickstart" aria-labelledby="quickstart-heading">
			...
			<ol class="guide-steps">
				<li><span class="guide-step__mark mono">01</span><div><h3>Sign in with an admitted identity</h3><p>{data.membership_detail} Use Google sign-in with the account you want to use. Claim the open team seat, or ask the commissioner to adjust the admission policy in <a href="/admin" data-gosx-link title="Sign-in required">/admin<span class="visually-hidden"> (sign-in required)</span></a>. Your Gridiron seat is separate from any Sleeper, ESPN, or Yahoo account.</p></div></li>
				<li><span class="guide-step__mark mono">02</span><div><h3>Read the live league rules</h3><p>Open <a href="/scoring" data-gosx-link title="Sign-in required">/scoring<span class="visually-hidden"> (sign-in required)</span></a> and check roster slots, scoring values, draft rounds, lineup locks, free agency, waivers, trades, and the weekly schedule. This is the current league configuration.</p></div></li>
				<li><span class="guide-step__mark mono">03</span><div><h3>Build your team's Big Board</h3><p>Rank players at <a href="/board" data-gosx-link title="Sign-in required">/board<span class="visually-hidden"> (sign-in required)</span></a>. The order is private to your team seat, shared by its primary and co-manager, and read first when the team is on the clock. Add backup names.</p></div></li>
				<li><span class="guide-step__mark mono">04</span><div><h3>Mark ready and choose autopick</h3><p>In <a href="/draft" data-gosx-link title="Sign-in required">/draft<span class="visually-hidden"> (sign-in required)</span></a>, use <strong>Mark ready</strong> when prepared and <strong>Mark not ready</strong> if that changes. Turn on autopick if you may miss your turn; it uses your board first and best available ADP when your board is empty.</p></div></li>
				<li><span class="guide-step__mark mono">05</span><div><h3>Set a lineup every week</h3><p>Use <a href="/team" data-gosx-link title="Sign-in required">/team<span class="visually-hidden"> (sign-in required)</span></a> to fill starting slots before each player's game locks. Bench, reserve, and injury states are visible there.</p></div></li>
			</ol>
		</section>
		...
		<section class="guide-section" id="commissioner-checklist" aria-labelledby="commissioner-checklist-heading">
			...
			<ol class="guide-checklist">
				<li><strong>Confirm identity and format.</strong><span>League name, season, timezone, teams, divisions, roster shape, and scoring on <a href="/scoring" data-gosx-link title="Sign-in required">/scoring<span class="visually-hidden"> (sign-in required)</span></a>.</span></li>
				<li><strong>Invite the right Google accounts.</strong><span>Use <a href="/admin" data-gosx-link title="Sign-in required">/admin<span class="visually-hidden"> (sign-in required)</span></a>, then verify each person signs in with the exact invited address.</span></li>
		...
		<section class="guide-section guide-section--accent" id="draft" aria-labelledby="draft-heading">
			...
			<div class="guide-card-grid guide-card-grid--three">
				...
				<article class="guide-card"><span class="section-index">BIG BOARD</span><h3>Your team's private queue</h3><p>Primary and co-manager drag players into one shared seat order at <a href="/board" data-gosx-link title="Sign-in required">/board<span class="visually-hidden"> (sign-in required)</span></a>. Picked players leave the available list; the draft tape and autopick use the same top available targets.</p></article>
		...
		<section class="guide-section" id="league-tools" aria-labelledby="league-tools-heading">
			...
			<div class="guide-card-grid guide-card-grid--three">
				<article class="guide-card" id="lineups"><span class="section-index">/TEAM</span><h3>Lineups</h3><p>Set starters and bench players in the Team Terminal, plus reserve or injury placements when the league config includes those zones. Set or auto-fill the best lineup before each player's game starts; locked slots cannot be edited after kickoff.</p><a href="/team" data-gosx-link class="guide-card__link" title="Sign-in required">Open Team Terminal →<span class="visually-hidden"> (sign-in required)</span></a></article>
				<article class="guide-card" id="waivers"><span class="section-index">/PLAYERS</span><h3>Waivers + free agency</h3><p>After the draft, undrafted players are free agents. You can sign one outright only in the dead window between the run that settles a week and the next week's first kickoff. Once that week's games are being played, the same player is won by claim in waiver order at the next daily run, so nobody takes a breakout by clicking first. A dropped player is on waivers until the clear window passes. File a claim in the Player Pool; mode, priority or FAAB, and resolve time are shown there.</p><a href="/players" data-gosx-link class="guide-card__link" title="Sign-in required">Open Player Pool →<span class="visually-hidden"> (sign-in required)</span></a></article>
				<article class="guide-card" id="trades"><span class="section-index">/TRADES</span><h3>Trades</h3><p>Send an offer, accept or decline incoming offers, and counter when needed. An accepted trade may enter a review window; the configured commissioner or league-veto policy decides what happens next.</p><a href="/trades" data-gosx-link class="guide-card__link" title="Sign-in required">Open Trade Desk →<span class="visually-hidden"> (sign-in required)</span></a></article>
				<article class="guide-card" id="scoring-rules"><span class="section-index">/SCORING</span><h3>Scoring</h3><p>Scoring values, roster shape, schedule, locks, waivers, and trade policy are the source of truth. Official corrected stats win over provisional news, and a closed week keeps its locked lineup and result.</p><a href="/scoring" data-gosx-link class="guide-card__link" title="Sign-in required">Read Rules + Scoring →<span class="visually-hidden"> (sign-in required)</span></a></article>
				<article class="guide-card" id="pickem"><span class="section-index">/PICKEM</span><h3>Pick'em</h3><p>Pick one team against the displayed market spread at <a href="/pickem" data-gosx-link title="Sign-in required">/pickem<span class="visually-hidden"> (sign-in required)</span></a>. Lines freeze Thursday, but each game stays pickable until its own kickoff. After you enter the week, a missed kickoff is a loss while every later game remains open. A tie against the spread is also a loss. Pick'em is its own W-L game; no fantasy roster seat is required.</p><a href="/pickem" data-gosx-link class="guide-card__link" title="Sign-in required">Open Pick'em HQ →<span class="visually-hidden"> (sign-in required)</span></a></article>
				<article class="guide-card" id="wire"><span class="section-index">/WIRE</span><h3>Signal Wire</h3><p>The Wire is a mixed-source alert surface for publisher feeds, community tips, and curated social posts. Signals stay provisional, never mutate fantasy scores, and show source/trust context. Market sightings are human-entered; this league does not log into or scrape another account.</p><a href="/wire" data-gosx-link class="guide-card__link" title="Sign-in required">Open Signal Wire →<span class="visually-hidden"> (sign-in required)</span></a></article>
			</div>
		</section>
		...
		<section class="guide-section" id="commissioner-hq" aria-labelledby="commissioner-hq-heading">
			...
			<a href="/commissioner" data-gosx-link class="button button--compact" title="Sign-in required">Open Commissioner HQ →<span class="visually-hidden"> (sign-in required)</span></a>
		</section>
		...
		<section class="guide-section guide-section--accent" id="migration" aria-labelledby="migration-heading">
			...
			<div class="guide-migration">
				...
				<ol class="guide-checklist">
					...
					<li><strong>Use each control where it actually lives.</strong><span>Use <a href="/admin" data-gosx-link title="Sign-in required">/admin<span class="visually-hidden"> (sign-in required)</span></a> for invites, seats, roster shape, draft order/clock, and announcements. Verify the resolved roster, scoring, schedule, waiver, and trade rules on <a href="/scoring" data-gosx-link title="Sign-in required">/scoring<span class="visually-hidden"> (sign-in required)</span></a> before inviting the league.</span></li>
