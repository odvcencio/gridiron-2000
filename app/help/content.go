...
// VerifiedSourceSHA is the exact origin/main source snapshot against which
// this corpus was reviewed for the weekly matchup-guidance slice (2026-09-09).
// It is deliberately a recorded provenance value for that slice, not a
// generated self-hash, unrelated-topic re-audit, or an age gate.
const VerifiedSourceSHA = "43c1c50dfbd493b01368d2c74959eb23de098749"
...
func buildCorpus() []Topic {
	...
	t = baseTopic("draft", "big-board-and-autopick", "Big Board and autopick", "Rank private draft targets for the shared team seat and understand exactly which order AUTO consumes.", []string{"big board", "board", "queue", "draft queue", "rank", "autopick", "auto pick", "adp"}, []string{"draft queue", "player queue", "my rankings", "what will autopick choose"}, []string{"draft board", "war board"})
	details(&t, "The primary or co-manager associated with a team seat; commissioner access is separate.", "A persisted seat association and available pool are required to edit a board.", "Dynasty/redraft; one private durable order belongs to each team seat.", "Loading, empty, no-results, saved, player-picked, disabled, permission-denied, stale, unavailable.", "The draft clock and player kickoff locks are runtime deadlines.", []string{"Search/browse the pool.", "Add, remove, and reorder targets for the shared team-seat board.", "Keep backups because picked players leave availability.", "Read Draft's AUTO explanation before leaving the clock unattended."}, "The board is private to the team seat; its primary and co-manager share the order. A commissioner sees only seat-level gap/count unless also a member of that seat.", "AUTO selects the shared seat board's first available target, then authoritative best available.", "Board edits are reversible while available; a pick follows commissioner correction rules.", "Saved ordering and search/filter context remain visible to both authorized seat managers.", "Unavailable player, stale ordering, permission, and transport failures may have an unknown outcome; reread the current board before retrying.", "Reread after another client change; if an action's outcome is unknown, refresh the board and activity before any retry. Use Draft to verify authoritative pick.", "Seat-scoped board persistence under the canonical primary-manager owner key, player availability, draft pick state, and autopick selector.", "The primary and co-manager see the same private seat board, while a commissioner who is not a seat member sees only readiness, presence, and board gap/count.", "/board")
	add(t)

	t = baseTopic("draft", "practice-draft", "Practice draft", "See what a live draft looks like before the real one: take picks on the clock in a copy of the draft room; nothing you do there is saved.", []string{"practice", "practice draft", "mock", "mock draft", "simulation", "rehearse", "bots", "try the draft"}, []string{"can I try the draft room", "mock draft", "test the draft", "draft simulator"}, []string{"scrimmage", "walkthrough"})
	details(&t, "A seated primary or co-manager, before the real draft starts.", "A team seat, and a real draft that has not started; a seatless commissioner cannot practice.", "Every league mode; the practice uses the league's real pool, draft order, roster shape, and pick clock.", "Available, unavailable (no seat, real draft live, draft complete), open, on-clock, complete.", "The practice pick clock is the league's real pick clock; a practice runs until the sandbox draft's final pick, until you leave, or until the real draft starts.", []string{"Open Draft and choose Practice the draft room, or go to /draft/practice.", "Choose a start round: early, middle, late, or specialists.", "Pick when you are on the clock; the other seats pick after a short pause.", "Leave whenever you like, or start again from any round."}, "A practice is private to your own sign-in; no one else sees it.", "Nothing is saved: no pick, readiness, autopick, or board change reaches the league.", "Leave or restart at any time; the real draft is untouched.", "The room says PRACTICE and that picks do not count; the real draft's start time stays visible.", "Once the real draft has started, the practice draft is gone: its page redirects to the real room, and an open practice ends with a link there.", "Return to Draft; the real room is exactly as you left it.", "Draft config, draft order, player pool, Big Boards, pick clock: an in-memory copy only.", "A manager practices the late rounds twice to see how kickers and defenses come off the board.", "/draft/practice")
	add(t)

	// Keywords carry "superflex", "punter", "punting", and "half PPR" (J6
	// F8, 2026-09-04 audit): a manager searching this league's own two
	// signature house rules — a SUPERFLEX slot and a startable punter —
	// used to get "No matching topic." for both.
	t = baseTopic("team", "lineups-locks-matchups-and-scoring", "Lineups, locks, matchups, and scoring", "Set the effective lineup before kickoff and read starter-only projections, AWAITING RELEASE, and final scoring honestly.", []string{"lineup", "starter", "bench", "lock", "kickoff", "matchup", "score", "scoring", "stat correction", "projection", "projected total", "projection week", "bench projection", "partial projection", "missing projection", "move starter", "start replace", "transfer", "superflex", "punter", "punting", "half PPR"}, []string{"when do lineups lock", "lineup deadline", "game lock", "fantasy points", "why is projection unavailable", "why is my projection missing", "how do I move a starter"}, []string{"starting lineup", "scorebug", "projection guidance"})
	details(&t, "Admitted viewers read matchups; an authorized manager changes only unlocked slots.", "Team association, selected week, and current week/slot lock state control mutations; the matching projection source controls forecast visibility.", "Configured regular-season or applicable phase; a selected week without a matching projection source keeps forecasts unavailable.", "Upcoming, lineup-editable, lineup-locked, live-provisional, source-stale, degraded, final, corrected, bye, unavailable; a source-week mismatch or partial forecast is not a zero.", "Each player's kickoff and league timezone govern that slot.", []string{"Open Team or Matchups and select the week you mean to inspect.", "Read the projection source/week label before treating a forecast as current; a mismatch leaves it unavailable.", "Read the starters-only projected total; the Bench disclosure is comparison context and never contributes to team score.", "Treat an em dash or partial/unavailable coverage as unknown, not zero; reread after the source refreshes.", "Before kickoff, use Team's Move / change or Start / replace control (or the transfer affordance when available) to place an eligible player in a starter slot.", "Save before kickoff and reread the saved lineup, lock, and scoring state.", "Use Matchups for provisional/final labels and reread after source change."}, "Roster visibility follows league policy; projections are not private authority.", "Only configured starters contribute to matchup/team scoring; bench projections are comparison context and never inflate the starter total. Locked slots cannot be changed normally; closed weeks pin effective starters/results.", "Unlocked Move / change and Start / replace controls can revise eligible destinations; kickoff locks the affected slot, and locked/final outcomes use commissioner correction.", "Local result names saved slot/player state and preserves week/team context; a projected total is numeric only when the selected week's required starter forecasts are known.", "Authorization, cross-week, lock, invalid-slot, and stale-state failures reject a lineup mutation; a projection-source failure, source-week mismatch, or missing forecast suppresses only the forecast, so a legal edit can still be saved, and unknown is never rendered as zero.", "Return to Team or Matchups, inspect selected week, source week, lock reason, and last-success age, then retry only when available; use an eligible Move / change or Start / replace control before kickoff.", "Roster config, schedule, lineup persistence, matching-week projection pool, stats ledger, matchup/correction state, and lineup movement processor.", "A selected week whose projection source names another week keeps projected totals unavailable, while a missing forecast for one starter leaves the total unknown and a bench forecast never adds to team score.", "/team")
	add(t)

	// Title and summary stay mode-neutral (J6 F8, 2026-09-04 audit): this
	// corpus entry serves every league, and a league that runs
	// performance-priority waivers, not FAAB, used to see its own waivers
	// topic named "...and FAAB" as if that were this league's rule. FAAB
	// stays a searchable keyword/alias below; helpIndexData
	// (page.server.go) states the current league's actual configured mode
	// on the rendered search result before either word appears.
	t = baseTopic("players", "players-free-agents-waivers-and-faab", "Players, free agents, and waivers", "Acquire an eligible player using this league's own configured claim rule — priority order or FAAB bid — and preserve filter context.", []string{"players", "free agent", "waiver", "claim", "FAAB", "budget", "priority", "add", "drop", "acquisition", "perf-priority", "performance priority"}, []string{"waiver budget", "bid units", "why can't I add a player", "how do waivers work"}, []string{"player pool", "wire pickup"})
	details(&t, "An authorized manager with roster capacity and an eligible player action.", "Team association, capacity/position eligibility, acquisition capability, and priority/FAAB units.", "The current league waiver mode; unsupported/source-unavailable modes are labeled.", "Free-agent-available, waiver-locked, claim-draft, claim-submitted, pending-run, processing, awarded, lost, canceled, roster-ineligible, insufficient-FAAB, priority-unavailable, source-unavailable.", "Exact waiver clear/process time, timezone, and relative text come from runtime.", []string{"Read pool freshness and eligibility.", "Add a true free agent in the dead window, or file a claim once the week's games are underway.", "Enter non-currency FAAB units when configured.", "Review drop, priority, and submitted state.", "Cancel/reorder only while permitted."}, "Claims/bids follow configured visibility; never expose another team's private bid.", "A claim is revalidated at processing; capacity, ownership, eligibility, and units may change.", "Edit/cancel only before the displayed boundary; awarded movement follows runtime finality.", "Submitted/awarded/lost/canceled reason and player/filter context are shown.", "Validation, stale, insufficient units, capacity, source, and permission failures spend nothing.", "Return to Players with waiver filter; reread state/last success before retry/cancel.", "Acquisition config, pool, roster, claim queue, processor clock, source freshness.", "Two claims can be submitted before processing; the configured priority or units decide the recorded result.", "/players")
	add(t)
	...
	t = baseTopic("pickem", "pickem", "Pick'em", "Choose against the displayed market line with per-game locks and an independent W-L record.", []string{"pickem", "pick'em", "spread", "ATS", "line", "tie", "missed", "kickoff", "picks"}, []string{"where did my pick go", "against the spread", "pick sheet", "football picks"}, []string{"picks", "confidence picks"})
	details(&t, "An admitted member with Pick'em capability for the current week/slate.", "Admission and the current slate; a fantasy team seat is not required.", "Current configured slate/source; unavailable lines become void/unavailable, not straight-up guesses.", "Not-open, open-unpicked, partially-picked, submitted, edited, locked, scored, corrected, missed, not-applicable, unavailable.", "Each game's kickoff and line-freeze rule are runtime deadlines in league-local time.", []string{"Read the market spread.", "Select a side.", "Save/submit per form semantics.", "Review per-game lock/result; later games remain open.", "Read W-L separately from fantasy scoring."}, "Pick visibility follows configured pre/post-lock policy.", "After week entry, a missed kickoff is a loss; a game tied against the spread is also a loss, and a void has no result.", "Edits stop at each game's runtime lock; scored/corrected results persist.", "Each game reports saved, locked, missed, loss, void, or scored state.", "Stale slate, unavailable line, validation, and authorization preserve unaffected picks.", "Return to Pick'em with week context, reread market/locks, retry open games only.", "Pick'em config, market/slate, schedule/kickoffs, entries, result processor.", "Thursday can lock while Sunday games remain open; entering the week does not lock the sheet.", "/pickem")
	add(t)
...
var roleChecklists = map[string][]ChecklistItem{
	"primary":    {{ID: "team-identity", Role: "primary-manager", Title: "Confirm team identity and seat", Detail: "Review display identity, roster shape, co-manager membership, and primary-only actions.", Predicate: "associated primary manager", ActionRoute: "/team"}, {ID: "lineup", Role: "primary-manager", Title: "Learn lineup locks", Detail: "Set unlocked starters before kickoff and read the exact lock reason.", Predicate: "associated primary manager", ActionRoute: "/help/lineups-locks-matchups-and-scoring"}, {ID: "board", Role: "primary-manager", Title: "Build the shared seat Big Board", Detail: "The primary manager and co-manager use one private team-seat order; read which target AUTO consumes.", Predicate: "draft capability and applicable phase", ActionRoute: "/board"}, {ID: "workflows", Role: "primary-manager", Title: "Review applicable workflows", Detail: "Review players, waivers, trades, Pick'em, and Blitz only when capability/phase applies.", Predicate: "capability and phase predicates", ActionRoute: "/players"}},
	"co-manager": {{ID: "shared-seat", Role: "co-manager", Title: "Confirm the shared team seat", Detail: "Read primary relationship, permissions, private/team-visible data, and detach implications.", Predicate: "admitted co-manager", ActionRoute: "/help/roles-primary-co-manager-and-commissioner"}, {ID: "board-truth", Role: "co-manager", Title: "Read the shared seat board", Detail: "The primary manager and co-manager read and write one private team-seat order; AUTO consumes that same order.", Predicate: "admitted co-manager", ActionRoute: "/board"}, {ID: "co-workflows", Role: "co-manager", Title: "Use the team's applicable workflows", Detail: "Review roster, lineup, acquisition, trade, Pick'em, and Blitz with disabled reasons.", Predicate: "capability and phase predicates", ActionRoute: "/team"}},
...
var commissionerChecklist = []ChecklistItem{
	...
	{ID: "roster-correction", Role: "commissioner-overlay", Title: "Correct a roster on a team's behalf", Detail: "Choose the team, review the named change, then confirm; give a required reason. Applies immediately; the team sees a one-time notice on their next visit.", Predicate: "commissioner capability and a completed draft", ActionRoute: "/admin?section=roster#roster-correction"},
...
// ChecklistProjection is the route-safe role projection for the Help index.
// ChecklistFor remains the reusable corpus primitive; this boundary decides
// which role the current viewer actually has and keeps commissioner capability
// orthogonal to team-seat membership. A visitor never receives an admitted
// member checklist merely because the public page can render one.
type ChecklistProjection struct {
	Role              string
	RoleLabel         string
	Heading           string
	EmptyMessage      string
	Items             []ChecklistItem
	CommissionerItems []ChecklistItem
	HasChecklist      bool
...
func checklistItemsForRole(items []ChecklistItem, role string) []ChecklistItem {
...
// checklistProjectionForViewer derives the Help index's visible cards from
// PublicEntryDataForViewer. That map is the same admission/seat/role boundary
// used by the product pages, so the help checklist cannot drift back to a
// hard-coded primary manager plus commissioner card.
func checklistProjectionForViewer(viewer, entry map[string]any, mode, phase string) ChecklistProjection {
	if entry == nil {
		entry = viewer
	}
	signedIn := boolFromMap(entry, "signed_in")
	admitted := boolFromMap(entry, "admitted")
	if !signedIn {
		// A demo/test viewer can still carry commissioner capability while
		// anonymous. Keep the base checklist gated on actual admission.
		admitted = false
	}

	projection := ChecklistProjection{
		Role:            "visitor",
		RoleLabel:       "SIGN-IN REQUIRED",
		Heading:         "Start with league access",
		EmptyMessage:    "Sign in and complete league admission to see the checklist for your current role.",
		HasCommissioner: boolFromMap(entry, "is_commissioner"),
	}
	if signedIn && !admitted {
		projection.Role = "admission-required"
		projection.RoleLabel = "ADMISSION REQUIRED"
		projection.Heading = "Complete league access"
		projection.EmptyMessage = "Complete league admission before using a team-seat checklist. The current page names the valid access route."
	}
	if admitted {
		projection.Role = "seatless"
		projection.RoleLabel = "SEATLESS MEMBER"
		projection.Heading = "What remains available"
		if boolFromMap(entry, "has_seat") {
			if boolFromMap(entry, "is_co_manager") {
				projection.Role = "co-manager"
				projection.RoleLabel = "CO-MANAGER"
				projection.Heading = "Use the shared team seat"
			} else {
				projection.Role = "primary"
				projection.RoleLabel = "PRIMARY MANAGER"
				projection.Heading = "Before the next action"
			}
		}
		projection.Items = ChecklistFor(projection.Role, mode, phase, false)
		projection.HasChecklist = true
	}

	if projection.HasCommissioner {
		// Use the resolved role for applicability, but expose only the
		// overlay items in the second card. A seatless commissioner therefore
		// receives seatless base guidance plus the same commissioner overlay;
		// a seated commissioner keeps their primary/co-manager base role.
		all := ChecklistFor(projection.Role, mode, phase, true)
		projection.CommissionerItems = checklistItemsForRole(all, "commissioner-overlay")
	}
	return projection
}
...
var migrationMappings = []MigrationMapping{
	...
	{Canonical: "Big Board", IncomingAliases: []string{"draft queue", "draft board", "rankings"}, Equivalent: "One private ordered list for a team seat can guide AUTO.", Difference: "The primary manager and co-manager share the seat board; a commissioner who is not a member of that seat sees only gap/count signals.", RuntimeSource: "Seat-scoped board persistence under the canonical primary-manager owner key, player pool, autopick selector.", Privacy: "Board order is private to the team seat and its authorized managers.", Consequence: "AUTO uses the shared seat board's first available target, then best available.", NextAction: "Build the team-seat board and verify Draft copy."},
...
var stateGuidance = map[string]StateGuidance{
	"loading":           {State: "loading", Why: "The read/action is still in progress.", Impact: "The value is not final yet.", Remaining: "Keep the route and prior safe content.", PreservedContext: "Preserve route, query, form, and focus.", NextAction: "Wait for local status.", Retry: "Do not duplicate a mutation while loading.", LastSuccess: "Not applicable while the read/action is still in progress."},
	"empty":             {State: "empty", Why: "The valid collection has zero items.", Impact: "Nothing is selectable yet.", Remaining: "Create/discovery and navigation remain available.", PreservedContext: "Keep league/team/week context.", NextAction: "Use the first meaningful create/discovery action.", Retry: "Retry only if source could change.", LastSuccess: "The current empty read is the latest collection result; no item timestamp is implied."},
	"no-results":        {State: "no-results", Why: "The current filter/query matches nothing.", Impact: "The collection may still contain records.", Remaining: "Clear/reset or change the query.", PreservedContext: "Preserve query, filters, tab, and anchor.", NextAction: "Clear filters or broaden the search.", Retry: "Retry with a new query; do not call collection empty.", LastSuccess: "The filtered read succeeded; no matching item does not mean the source failed."},
	"pending":           {State: "pending", Why: "Work awaits a deadline, review, or processor.", Impact: "Final consequence has not occurred.", Remaining: "Read submitted state/time and unaffected actions.", PreservedContext: "Preserve submitted form and owning route.", NextAction: "Wait for displayed boundary.", Retry: "Do not resubmit; edit/cancel only when shown.", LastSuccess: "Submission is accepted for processing; no final success is recorded yet."},
	"saved":             {State: "saved", Why: "Persistence succeeded.", Impact: "The named object has the saved effect.", Remaining: "Continue and use undo/edit if offered.", PreservedContext: "Keep week/team/filter/focus on result.", NextAction: "Read exact saved consequence.", Retry: "Refresh to reconcile; do not replay.", LastSuccess: "This saved result is the latest successful mutation; the owning page may show its receipt time."},
	"locked":            {State: "locked", Why: "A time/rule/final boundary prevents change.", Impact: "The mutation cannot apply now.", Remaining: "Read locked reason and permitted route.", PreservedContext: "Preserve object/week/form/query.", NextAction: "Wait for unlock or use commissioner path.", Retry: "Retry only after boundary changes.", LastSuccess: "No successful mutation occurred at this locked boundary; use any runtime-provided prior receipt."},
	"disabled":          {State: "disabled", Why: "Role, prerequisite, or capability prevents action.", Impact: "Submission must not mutate state.", Remaining: "Explanation and prerequisite remain available.", PreservedContext: "Keep route/task context.", NextAction: "Complete prerequisite or ask authorized operator.", Retry: "Do not retry as another identity.", LastSuccess: "Not applicable: a disabled submission has no successful mutation receipt."},
	"stale":             {State: "stale", Why: "Last-success data is beyond freshness window.", Impact: "Snapshot informs but is not live.", Remaining: "Use unaffected fields and age.", PreservedContext: "Preserve filters/week/object.", NextAction: "Manual refresh or wait for source.", Retry: "Bounded retry; never fabricate fresh value.", LastSuccess: "The owning page supplies the last-success time and age; this guide never invents them."},
	"degraded":          {State: "degraded", Why: "A source/partial read failed while last-good data remains.", Impact: "Some fields are unknown/provisional.", Remaining: "Preserve unaffected content.", PreservedContext: "Preserve task/query/form/object.", NextAction: "Retry affected source or use alternate route.", Retry: "Do not replace unknown with zero.", LastSuccess: "The owning page supplies the retained source's last-success time and age."},
	"offline":           {State: "offline", Why: "Source/service is knowingly offline or fallback is serving.", Impact: "Live enrichment/mutation may be unavailable.", Remaining: "Use explicitly labeled rehearsal/cache content.", PreservedContext: "Preserve route and pending inputs.", NextAction: "Inspect source and wait for recovery.", Retry: "Retry after source reports available.", LastSuccess: "Use the owning page's last-success time and fallback-source label when present."},
	"unavailable":       {State: "unavailable", Why: "No reliable value exists for the scope.", Impact: "The source cannot authorize a fact/action.", Remaining: "Use unaffected routes and escalation.", PreservedContext: "Preserve league/team/week/filter.", NextAction: "Wait, retry, or use alternate route.", Retry: "Bounded retry; never fabricate zero/live.", LastSuccess: "No last-success value is available for this scope."},
	"failed":            {State: "failed", Why: "An attempted read/action failed.", Impact: "The effect may be unknown until the owning state is reread.", Remaining: "Safe form/query and unaffected content remain.", PreservedContext: "Preserve return target/query/form/focus.", NextAction: "Read local error, refresh/reread the owning route, and compare activity.", Retry: "If the outcome is unknown, reread before retrying; never replay a stale mutation.", LastSuccess: "Use the owning page's last-success value for reads; mutation outcome remains unknown until reread."},
	"permission-denied": {State: "permission-denied", Why: "Identity lacks authorization.", Impact: "Protected data/mutation is not exposed/applied.", Remaining: "Valid request/help route remains.", PreservedContext: "Preserve only safe same-origin context.", NextAction: "Use admission/role request path.", Retry: "Do not retry as another role or leak details.", LastSuccess: "Use only a safe last-success value supplied by the owning page; protected details stay hidden."},
	"not-applicable":    {State: "not-applicable", Why: "Mode, phase, role, or capability makes topic irrelevant.", Impact: "There is no obligation/control.", Remaining: "Read predicate and continue applicable checklist.", PreservedContext: "Preserve route if opened directly.", NextAction: "Return to applicable task/phase.", Retry: "No retry unless capability changes.", LastSuccess: "Not applicable for this mode, phase, role, or capability; no last-success value is implied."},
}
...
func ContextualFieldHelp(topicID, field string) map[string]string {
	...
	if !ok {
		return map[string]string{
			...
			"next_action":    "/help",
			"topic_id":       "",
			"action_label":   "Open the Help center →",
		}
	}
	...
	case "validation", "validation-error", "error":
		help = fmt.Sprintf("Validation for %s uses the owning action %s. %s Recovery: %s", topic.Title, topic.ActionRoute, topic.Failure, topic.Recovery)
	}
	return map[string]string{
		"field": normalized, "label": label, "help": help,
		"runtime_source": source, "next_action": next,
		"topic_id": topic.ID, "action_label": ActionRouteLabel(topic.ActionRoute),
	}
}
