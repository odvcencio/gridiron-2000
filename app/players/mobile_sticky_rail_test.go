...
// wave 7b) rather than relying on aria-label alone. Both the initial
// Page() render and the PlayerPoolRegion fragment (app/players/page.gsx)
// carry the identical contract, since either one can be the search box a
// manager actually sees depending on whether a region swap has run yet.
...
// shows a numeric pad with a "done" key instead of "go". Checked in both
// render copies (Page() and PlayerPoolRegion), same reasoning as the
// search-field test above.
func TestPlayersFAABBidCarriesANumericKeyboardAndARealLabel(t *testing.T) {
	...
	for _, want := range []string{
		...
		`<label class="visually-hidden" for={"players-bid-sync-" + player.id}>{"Bid FAAB for " + player.name}</label>`,
		`<input id={"players-bid-sync-" + player.id} type="number" inputmode="numeric" pattern="[0-9]*" enterkeyhint="done" name="bid" min="0" max={data.my_faab_remaining} placeholder="Bid FAAB"></input>`,
	} {
		if !strings.Contains(source, want) {
...
// wraps the position filters and search form in both render copies, the
// list is followed by a #pool-search back-to-top link, and the shared
// stylesheet's wave 7b block pins that rail under the fixed top bar with
// a sticky position at phone/tablet width — the fix for the pre-wave-7b
// audit finding that re-filtering cost a 1200px scroll back up.
func TestPlayersFilterRailIsStickyWithABackToTopLink(t *testing.T) {
	...
	source := string(page)
	if got := strings.Count(source, `<div class="pool-filter-rail" id="pool-search">`); got != 2 {
		t.Errorf(`page.gsx has %d copies of the pool-filter-rail wrapper, want 2 (Page() and PlayerPoolRegion())`, got)
	}
	if got := strings.Count(source, `<a class="access-link pool-back-to-top" href="#pool-search">`); got != 2 {
		t.Errorf(`page.gsx has %d back-to-top links, want 2 (Page() and PlayerPoolRegion())`, got)
	}
