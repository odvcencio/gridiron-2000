...
func WaiverDeskRegion() Node {
	return <section class="player-pool">
		...
		<ol class="waiver-order-strip"><Each of={data.waiver_order} as="slot"><li aria-current={slot.mine}><span class="mono">{slot.position}</span><span title={slot.name} aria-label={slot.name}>{slot.abbr}</span></li></Each></ol>
