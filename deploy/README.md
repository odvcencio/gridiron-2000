...
then compile and validate the fleet publication:
...
Review all generated files and the operator-checklist.md before provisioning
real Secrets, DNS, and OAuth. The checklist gives the exact callback for each
instance as <public_origin>/auth/google/callback. The fleet definition has no
Secret values or member identities. Provision those private values and external
records only after review, then apply the namespace, PVC, ConfigMap, Secret,
Service, Deployment, middleware, HTTPS ingress, and HTTP redirect in the
reviewed order.

An existing immutable release has a different order: build and publish one
image once, pin and record its immutable digest, pass the Stable Kernel (SK)
canary acceptance gates, then roll that identical digest through the flagship
...
automatically adopted or regenerated by this lane. Review generated output and
apply it explicitly when choosing this workflow.

The shared statrelay is the sole owner of the real TANK01_API_KEY; league
application Secrets receive only its configured relay origin. Generated
local-path PVCs are ReadWriteOnce and node-local. Inspect the StorageClass
reclaim policy before deleting a claim, and arrange backups and node-loss
recovery separately; this local-path setup makes no HA or backup guarantee.

## Commissioner HQ v1 fleet topology

The generated fleet publication uses schema version 2. Each instance has one
required nullable `commissioner_hq` object: `null` is a nonparticipant, while
an object supplies the unique `league_id`, nonnegative `order`, safe-token
`accent` and `key_id`, and `host` fields. A participating fleet has exactly
one host. The participant set, ordered by `order`, is the complete HQ v1
registry; the host is the only browser consumer.

For every participant, review its generated `secret.example.yaml` and fill
`COMMISSIONER_HQ_PROVIDER_SECRET` with a newly generated value. On the host,
review `hq-client-secret.example.yaml` and fill each
`COMMISSIONER_HQ_V1_SECRET_<INSTANCE>` with exactly the matching participant
provider Secret value. These are read-only, scoped HMAC credentials, not
legacy `COMMISSIONER_HQ_TOKEN`/peer-mesh tokens, Tank01 keys, sessions, OAuth
credentials, or browser identities. The registry ConfigMap contains only
origins, canonical links/capabilities, key IDs, and `secret_env` references;
never put a credential value in `fleet.json` or the registry.

First-install apply order is: namespaces; PVCs and league/registry ConfigMaps;
all application, provider, and host-client Secrets; Services; participant
NetworkPolicies; Deployments; security middleware; HTTPS Ingresses; then HTTP
redirects. Apply both sides of every provider/client pair before rolling an
affected pod. For deliberate rotation, update the matching provider and host
client Secret resources together, then roll all affected Deployments in one
planned window and verify the host's registry reads; do not rotate one side
or copy a value into another participant's pair. Public Services keep port 80
targeting the named application port 8080; public Ingresses route only to that
Service port. Provider Services are private ClusterIP endpoints on 8091, and
their NetworkPolicies admit 8091 only from the exact host namespace and pod
labels.

For an existing immutable release, use the generated operator checklist for
the schema-v2 apply and SK-first canary order. The hand-authored manifests
under `deploy/k8s/**` are not adopted or regenerated by fleetgen; do not mix
their legacy peer/token wiring into a generated schema-v2 bundle.

### Existing-fleet adoption preflight
...
content, member identities, OAuth data, and Secret values. It is intentionally
limited to resource names, public origins, the exact immutable image digest,
legacy/v1 wiring booleans, and explicit PVC/ConfigMap/Secret preservation
...
drift, missing state-preservation acknowledgements, duplicate JSON keys, or
member-identity input. It reads no Kubernetes Secret object and emits no
credential-bearing value. A ready plan classifies the existing PVC, live
league ConfigMap, and application Secret as preserve-only; the generated
provider Service, provider NetworkPolicy, host registry ConfigMap, and host
client Secret as explicit additions; and legacy peer/token wiring as deferred
...
the Secret workflow without displaying them, apply both sides of each pair,
apply the generated private Services/NetworkPolicies/registry ConfigMap,
patch the existing Deployments with the generated v1 listener contract while
retaining the live PVC/league ConfigMap/application Secret, and roll Stable
Kernel first. Verify `/api/health` reports `commissionerHQV1Configured` and
`commissionerHQV1Listening` true, the host registry reaches each provider, and
the existing `/commissioner` behavior remains healthy before rolling flagship.
Retire the legacy peer/token wiring only after both instances pass that
canary; no step in this source lane authorizes a deployment.
