...
## [release-2026.09.18-4fa6d3a-keep-rostered] — 2026-09-18

Scope: Keep every rostered player in the player pool, so a released player never vanishes from the roster that owns him.
...
## [release-2026.09.16-2774d79-tabbar-icons] — 2026-09-16

Scope: Draw the mobile tab bar icons instead of borrowing them from the font.
...
Scope: Show roster moves on Matchups immediately and explain a starter's projection before kickoff.
...
- The wheel panel puts one player on each line. The merged kicker and punter piece separated its two players with a middot, which could wrap to the start of the second line.
- The matchup wheel keeps its ten pieces, and the tenth now holds the kicker and the punter at half weight each. The arc moves when either one plays instead of sitting still until the slower of the two finishes, and it still only reads complete when both are done — the two quarters average down, never up. A roster that fields only a kicker completes that piece on the kicker alone.
- Every wheel arc now explains itself on hover or focus: the slot, its players, and the score each has actually posted. The text is live-bound rather than a title attribute, so it cannot freeze at its render-time value during a game.
...
### Fixed
- The points tooltip no longer makes the matchups page scroll sideways on a phone. It was hidden with `visibility`, which still occupies layout, and it is wider than the cell it hangs off — so at a 390px viewport the document measured 413px while the tooltip showed nothing. It is now taken out of layout entirely when hidden.
- The points tooltip's total no longer reuses the score's own live-update key. Binding the same key twice meant every selector looking for one score cell per starting slot found two.

## [release-2026.09.10-192ea74-season9] — 2026-09-10

Scope: understandability sweep: the live status chip no longer shows internal vocabulary — it read LEDGER, which named nothing a manager could act on and meant BOTH ends of a week (before anything kicks off, and once the week is posted and official), and now reads SCHEDULED, LIVE, UNDERWAY, AWAITING FINAL or FINAL, one situation each, while PAUSED (which described our own delayed feed, not a stopped game) now says FEED DELAYED; the H## chip beside every player explains itself, house rank being this league's own value ranking for its roster shape and team count, which appeared on every row across the lineup and the pool with no explanation anywhere in the app; and a starter's points now explain themselves in a tooltip on the score itself rather than inside a disclosure, revealed on hover AND focus so a phone tap or a Tab reaches it, described to a screen reader, live-bound so it cannot drift from the score during a game, and anchored toward the middle of the row after measurement showed a right-anchored tooltip running 51px off the left of a 390px viewport. The raw state tokens are unchanged in the polled JSON contract. No schema change.

### Changed
- A starter's points now explain themselves in a tooltip on the score itself, instead of inside a disclosure you had to open. It reveals on hover and on focus — focus matters as much as hover, because a phone has no hover, and a tap or a Tab now reveals it; the cell is described by the tooltip so a screen reader announces the explanation with the number. The rules stay a live-bound text node rather than a title attribute: the poll patches text, so a title would freeze at its render-time value and drift from the score beside it during a game. Each side of the matchup opens its tooltip toward the middle of the row, after measurement showed a right-anchored tooltip running 51px off the left of a 390px viewport and losing its first word.
...
Scope: phone reach, matchup legibility, and two injury-feed corrections: content at the bottom of a page is no longer parked under the fixed tab bar where it cannot be tapped (the page reserved the space but never told the browser where the usable viewport ends, so every anchor jump, focus move and scroll-into-view still landed beneath it); home and away are marked on every matchup and the win percentage names the team it describes, following the viewer's own side rather than assuming it is always home; text no longer strands a short word alone on its own line, measured at 390px and reduced from 75 stranded words to none on the matchups page; the injury chip updates on the same poll as the score it sits beside and reads correctly to a screen reader; Rams, Commanders and Jaguars players can use injured reserve again, after the weekly injury report was found to be queried with the player pool's team spelling against rows stored under the mirror's — the sixth site of that same mismatch; and the compact injury chip no longer renders a whole sentence for a designation its second, drifted derivation did not know. No schema change.

### Fixed
- Rams, Commanders, and Jaguars players can use injured reserve again, and their injury chip reads both feeds. The weekly injury report is queried by team, and the query passed the player pool's Tank01 spelling ("LAR") against rows stored under nflverse's ("LA"), compared as a raw string — so the report returned nothing for those three teams. IR placement refused every one of their players with a message blaming the player, while the player's own chip read Out, and the reconciled injury status quietly fell back to a single feed. This was the sixth call site of that same abbreviation mismatch.
- The compact injury chip no longer renders a whole sentence. Its code was derived a second time by a private switch that had drifted out of step with the canonical vocabulary and had no case for "physically unable to perform" or "suspended"; both now abbreviate as they should.
- On a phone, content at the bottom of a page is no longer parked underneath the fixed tab bar where it cannot be tapped. The page reserved space below its last content, but never told the browser where the usable viewport ends, so anything the browser scrolled to itself — an in-page anchor jump, a focused control, a form field the keyboard scrolls into view — still landed under the bar. The team page's contextual Help link sat squarely beneath it: tapping the link hit the MATCHUPS tab instead and navigated away.
- Text no longer strands a short word alone on its own line. Measuring every wrapped line at 390px found 76 of them across two screens, and they were not the mid-word breaks an earlier pass assumed — they were widows, with two distinct causes. Prose now uses `text-wrap: pretty`, which reconsiders the last few line breaks to avoid stranding a word. Compact labels that are one atomic unit ("WR · CHI", "PROJ 9.5") no longer wrap at all, because their grid cells measure 37 to 46 pixels and no wrapping rule can fix a box narrower than the label it holds. The starter ledger's provenance line also dropped its "Stats: " prefix, which was pushing every one of those lines onto a third line inside a 123-pixel column. The matchups page went from 75 stranded words to none, with no new horizontal overflow.
- Home and away are now marked on every matchup, and the win percentage says whose it is. The two sides read as interchangeable, and a bare percentage gave no clue which team it described or who was favoured. Around-the-league cards report the home side's chance and now name it; the featured card names the side it is describing and marks which of the two is hosting, following the viewer's own team rather than assuming it is always home.
...
Scope: week-1 projection truth and text wrapping: a starter whose game has finished is no longer projected as though the whole game were still ahead of them (the projection consulted only the live poller, whose entry for a game disappears once its window closes, so a defense that finished with 12.0 real points showed a projection of 23.8 — its points plus its entire weekly forecast stacked on top); the projection now reads the schedule too, and a finished game projects nothing further, which also un-inflates every team total and the win probabilities computed from them; Rams, Commanders and Jaguars starters were hit by that same doubled projection even while the poller had their game, the fifth and last call site of that abbreviation mismatch; and words now wrap whole instead of breaking mid-word and stranding a stray letter or two, with machine strings (invite URLs, email addresses, console identifiers) deliberately still breaking anywhere because they have no word boundary to wrap at. No schema change.
...
Scope: week-1 scoring truth: team defenses score at all (a D/ST was addressed by a display name the pool and both stat sources spelled differently, so no defense ever joined its own stat line); FINAL now waits for every starter's game instead of the first one, with an UNDERWAY state that keeps the projection and shows real points; a finished game shows its result beside a starter; Rams, Commanders and Jaguars starters resolve their live game again; an explicit 0.0 now requires positive evidence a game began; defensive scoring gains points-allowed and yards-allowed ladders plus forced fumbles, blocked kicks, defensive two-point returns and special-teams touchdowns, effective now; return touchdowns score from the weekly ledger, not only while the poller is up; any matchup opens at full page width; and a starter's points explain themselves rule by rule on both the matchups and team pages. No schema change.
...
- A starter's points now open a breakdown of where they came from, rule by rule — "Interception x3 6.0 · Sack x2 2.0 · 7-13 points allowed 4.0" — inside the ledger disclosure that already opens on a player. On the matchups page it updates on the same poll as the score it explains; on the team page it sits in the player's stat tip under "Scored this week", kept plainly separate from the projection breakdown already there, because one is a forecast and the other is what actually happened. It appears only once a real score has posted.
