...
- A commissioner should never have to force a week closed. A week whose clean conditions never go green — a game the feed never flips final, a stat ledger that caught up but never reached the 24 hour settle clause — now settles by itself at a deadline: three days past its own last kickoff, or the moment the next week starts being played, whichever comes first. The close is recorded as having settled on degraded data, naming how many games the feed reported final. Force close remains, for closing a week sooner rather than rescuing a stuck one. One case still holds deliberately and says so: if the stat ledger has not fetched since the week's own games, closing would post a silently wrong score for every team, so that one waits for a person.
