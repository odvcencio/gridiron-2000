...
| `READY TO CLOSE` | Every known real game is final and the player ledger satisfies the close gate. | Nothing to do: the week closes itself on the next roster-ops tick. Close it by hand only to close it sooner. |
...
A player's `Hist` line shows their previous season, scored under this league's own scoring rules. It never shows a generic market number. Gridiron rescores every QB, RB, WR, TE, and K's previous-season weekly stats (2025, for the 2026 season) through the same engine that scores the current season.
...
- An RB, WR, or TE spends a FLEX slot before a SUPERFLEX slot. This order leaves SUPERFLEX open for a QB to claim.
...
- A tied VORP breaks first by market ADP, then by name, then by player ID. A missing ADP sorts last.
...
`LIVE_MAX_INFLIGHT` still bounds concurrent box fetches, and `LIVE_DAILY_BUDGET` still caps box-score fetches per instance per day (a scoreboard or games-list call is never charged against it — `STATRELAY_DAILY_BUDGET` meters their upstream cost). `LIVE_POLL_INTERVAL` is the deprecated alias for `LIVE_SCOREBOARD_INTERVAL`; a tracked deploy manifest never sets it, but a self-hosted instance that still does gets it read as `LIVE_SCOREBOARD_INTERVAL`, with a startup log line naming the mapping.

**The verified Ultra quota:** the RapidAPI Tank01 Ultra listing caps at 15,000 requests per day, soft-limited — RapidAPI bills overage at $0.01/request instead of blocking, and returns no `429` on quota exhaustion, so the app's own 429 circuit breaker never fires on overage. `STATRELAY_DAILY_BUDGET` (default `13000`, on the shared relay) is therefore the real wallet guard; `LIVE_DAILY_BUDGET` (default `9000` per app instance, box fetches only) sits under it with headroom held back for other instances and endpoints sharing the relay. The layer-1 arithmetic after the `getNFLScoresOnly` adoption: an 11-hour Sunday window at the 10s tick costs about 4.0k scoreboard calls (the relay caches at `STATRELAY_SCOREBOARD_TTL`, default 10s, so more instances do not multiply this) plus about 0.7k listing calls at the 60s reuse — roughly 4.7k, against about 4.0k listing-only before, with the whole slate's score/clock/possession in return. Box fetches per 13-game Sunday still run about 9.6k (64%) at the tier ceilings, and the scoreboard's change gate plus possession freshness pull the realistic day well under that. The bound is enforced, not hoped for: `LIVE_DAILY_BUDGET` (9,000) caps the box side, so the worst combined day is about 13.7k — and `STATRELAY_DAILY_BUDGET` (13,000) truncates total upstream before the quota line regardless. Every case bills $0.

### Free-live profile

`LIVE_PROFILE=free` (and, on the shared relay, `STATRELAY_PROFILE=free`) trades the 10-second Ultra experience for a slow-live feed on Tank01's free BASIC tier: 1,000 requests per month, **hard-limited** — a `429` at the line, not billed overage. The profile sets `LIVE_SCOREBOARD_INTERVAL=30m`, `LIVE_BOX_BASELINE=6h`, `LIVE_BOX_FAST=6h`, and `LIVE_DAILY_BUDGET=20`; any of the four still yields to an explicitly set variable.
...
| Week close says games are unfinished or stats are stale. | Wait. The week turns over by itself at the first Tuesday midnight, league time, after its own last kickoff, and records that it settled on degraded data. Force close only to close it sooner. |
| Week close says the stat ledger has not fetched since this week's last game finished. | This is the one case that waits for you. Closing would score the week from a partial ledger. A Monday night game is still being played at Tuesday midnight, so this is normal for an hour or two after it; the week settles on the first check once the ledger catches up. If it persists, fix the stat feed — force close only as a recorded commissioner ruling that you accept those scores. |
