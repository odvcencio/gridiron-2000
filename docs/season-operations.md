...
| Week close says games are unfinished or stats are stale. | Wait. The week closes itself at its deadline — three days past its own last kickoff, or when the next week starts being played, whichever is first — and records that it settled on degraded data. Force close only to close it sooner. |
| Week close says the stat ledger has not fetched since this week's games. | This is the one case that waits for you. Closing would score the week from stats that predate its own games. Fix or wait for the stat feed, then let the normal close run; force close only as a recorded commissioner ruling that you accept those scores. |
