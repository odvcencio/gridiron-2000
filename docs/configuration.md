...
Other environment variables configure private identity policy, credentials, providers, storage, or runtime behavior; they are not part of the public `league.json` schema. See [`.env.example`](../.env.example) and the README configuration map. This includes the `LIVE_*` regular-season live-scoring variables (`LIVE_PROFILE`, `LIVE_SCORING_ENABLED`, `LIVE_SCOREBOARD_INTERVAL`, `LIVE_BOX_BASELINE`, `LIVE_BOX_FAST`, `LIVE_MAX_INFLIGHT`, `LIVE_DAILY_BUDGET`, `LIVE_REPLAY_FIXTURE`, `LIVE_REPLAY_STEP`, `LIVE_REPLAY_ALLOW_PRODUCTION`; `LIVE_POLL_INTERVAL` is a deprecated alias for `LIVE_SCOREBOARD_INTERVAL`) and `statrelay`'s own `STATRELAY_PROFILE`, `STATRELAY_DAILY_BUDGET`, `STATRELAY_BOX_LIVE_TTL`, and `STATRELAY_SCOREBOARD_TTL` — none override a `league.json` field, so none appear in the table above. `LIVE_PROFILE=free` and `STATRELAY_PROFILE=free` swap in a slower, free-Tank01-tier cadence for every `LIVE_*`/`STATRELAY_*` default at once; any variable set explicitly still wins over the profile. See [Game day](season-operations.md#game-day) and its "Free-live profile" subsection for the states, precedence, status-code rule, kill-switch procedure, and free-tier request arithmetic those variables control.
