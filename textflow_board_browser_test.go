...
// TestBrowserBoardPoolRowClampsCleanly is the text-flow wave's browser
// check (2026-09-05) for the Big Board's available-pool row player name
// and detail line (both keep the one-line row contract, maxLines=1): a
...
// desktop width.
func TestBrowserBoardPoolRowClampsCleanly(t *testing.T) {
	if testing.Short() {
	...
	for _, width := range []int64{390, 1440} {
		signInBrowserSeat(t, ctx, child, bot, "/board", width, 900)
		if err := chromedp.Run(ctx, chromedp.WaitVisible(".pool-player__text", chromedp.ByQuery)); err != nil {
			t.Fatalf("no board pool row at %dpx: %v", width, err)
		}
		var found struct {
			Name   bool `json:"name"`
		...
		if !found.Name || !found.Detail {
			t.Fatalf("board pool row name/detail not found at %dpx: %+v", width, found)
		}
