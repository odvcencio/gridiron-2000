...
| `LIVE_BOX_BASELINE` | `60s` | How often an in-progress game's box score is fetched at minimum, regardless of the wire trigger below |
| `LIVE_MAX_INFLIGHT` | `4` | Maximum concurrent in-progress game fetches per instance |
| `LIVE_DAILY_BUDGET` | `5000` | Per-instance daily cap on live box-score fetches (a games-list call is never charged against it); sized against the verified Ultra quota (15,000 requests/day, soft-limited) with headroom held back for `STATRELAY_DAILY_BUDGET` |
...
| `STATRELAY_DAILY_BUDGET` | `0` (unlimited) | `statrelay`'s own daily cap on relayed upstream Tank01 calls, shared by every league instance behind it; the tracked manifest sets `10000` against the verified Ultra quota — the real wallet guard, since RapidAPI bills overage instead of returning a 429 |
