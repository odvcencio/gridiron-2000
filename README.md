...
- A practice draft at `/draft/practice`: before the real draft, a seated manager sees what a live draft looks like by taking picks on the clock in a private copy of the room, in their real seat, against bots playing the other seats from their real Big Boards. It uses the real pool, draft order, and pick clock, starts from round 1, 5, 10, or 15, runs until the manager leaves or the sandbox draft ends, and saves nothing. Once the real draft starts, the practice draft is gone.
...
- A `/players` waiver desk: browse the pool, file claims, and bid FAAB (Free Agent Acquisition Budget) or use priority order, with private per-team receipts and a commissioner view of every outcome. An outright signing exists only in the dead window between the run that settles a week and the next week's first kickoff; once that week's games are being played, every acquisition is a claim resolved in waiver order at the next daily run, and an agreed trade runs at that same run rather than the instant its review clock expires. Each row shows what the player actually scored in the most recent completed NFL week, under this league's own scoring, and the pool leads with that number. `?sort=rank` returns to the market rank order.
- `/pickem`: an independent against-the-spread game with per-game locks, season standings, and a weekly leaderboard. Each week's sheet is that week's permanent record: once a game kicks off, its row names every entrant's call and how the call graded, and a settled week reads as the record rather than a form.
...
Requirements: Go 1.26 and GoSX v0.56.3.
