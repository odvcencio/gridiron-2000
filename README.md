...
- A `/players` waiver desk: browse the pool, file claims, and bid FAAB (Free Agent Acquisition Budget) or use priority order, with private per-team receipts and a commissioner view of every outcome. Each row shows what the player actually scored in the most recent completed NFL week, under this league's own scoring, and the pool leads with that number. `?sort=rank` returns to the market rank order.
